...
var OpenAIService = class {
  ...
  async generateTitle(content, prompt) {
    ...
    try {
      ...
      return ((_c = (_b = (_a3 = response.choices[0]) == null ? void 0 : _a3.message) == null ? void 0 : _b.content) == null ? void 0 : _c.trim()) || null;
    } catch (error) {
...
var OllamaService = class {
  ...
  async fetchModels() {
    ...
    try {
      ...
      return ((_a3 = data.models) == null ? void 0 : _a3.map((model) => model.name || model.model)) || [];
    } catch (error) {
...
var _AutoTitleSettingTab = class _AutoTitleSettingTab extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("AI Provider").setDesc("Choose which AI provider to use for title generation").addDropdown((dropdown) => dropdown.addOption("ollama", "Ollama").addOption("openai", "OpenAI").setValue(this.plugin.settings.provider).onChange(async (value) => {
      this.plugin.settings.provider = value;
      await this.plugin.saveSettings();
      this.display();
    }));
    if (this.plugin.settings.provider === "openai") {
      new import_obsidian2.Setting(containerEl).setName("OpenAI API Key").setDesc("Enter your OpenAI API key to enable AI title generation").addText((text) => text.setPlaceholder("sk-...").setValue(this.plugin.settings.openaiApiKey).onChange(async (value) => {
        this.plugin.settings.openaiApiKey = value;
        await this.plugin.saveSettings();
      }));
    }
    if (this.plugin.settings.provider === "ollama") {
      new import_obsidian2.Setting(containerEl).setName("Ollama URL").setDesc("Enter your Ollama server URL").addText((text) => text.setPlaceholder("http://100.84.149.35:11434").setValue(this.plugin.settings.ollamaUrl).onChange(async (value) => {
        this.plugin.settings.ollamaUrl = value;
        await this.plugin.saveSettings();
        _AutoTitleSettingTab.modelsRefreshed = false;
        this.validateAndFetchModels();
      }));
      new import_obsidian2.Setting(containerEl).setName("Ollama Model").setDesc("Select the model to use for title generation").addDropdown((dropdown) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Auto-rename new files").setDesc("Automatically generate titles for new files").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoRename).onChange(async (value) => {
      this.plugin.settings.autoRename = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Only rename Untitled files").setDesc('When auto-rename is enabled, only rename files that have "Untitled" in their name, and only when the file is left or closed').addToggle((toggle) => toggle.setValue(this.plugin.settings.onlyRenameUntitled).onChange(async (value) => {
...
var LoadingToast = class {
  ...
  show() {
    this.hide();
    this.notice = new import_obsidian3.Notice("Generating title...", 0);
  }
  hide() {
    if (this.notice) {
      this.notice.hide();
...
var TitleGenerator = class {
  ...
  async generateTitleForFile(file, isAutoRename = false) {
    ...
    if (this.plugin.settings.provider === "openai" && !this.plugin.openaiService.isConfigured()) {
      new import_obsidian4.Notice("OpenAI not configured. Please set your API key in settings.");
      return;
    }
    if (this.plugin.settings.provider === "ollama" && !this.plugin.ollamaService.isConfigured()) {
      new import_obsidian4.Notice("Ollama not configured. Please set your Ollama URL in settings.");
      return;
    }
    try {
      let content;
      try {
        ...
        if (!isAutoRename) {
          new import_obsidian4.Notice("Cannot read file content - may be a binary file");
        }
        return;
      }
      if (content.trim().length < 10) {
        if (!isAutoRename) {
          new import_obsidian4.Notice("File content is too short to generate a meaningful title");
        }
        return;
      }
      if (content.length > 1e5) {
        if (!isAutoRename) {
          new import_obsidian4.Notice("File is too large for title generation");
        }
        return;
      }
      ...
      if (!rawResponse) {
        new import_obsidian4.Notice("Failed to get response from AI service");
        return;
      }
      const extractedTitle = TitleExtractor.extractTitle(rawResponse);
      if (!extractedTitle) {
        new import_obsidian4.Notice("Failed to extract valid title from AI response");
        console.log("Raw AI response:", rawResponse);
        return;
      }
      const cleanTitle = TitleExtractor.validateAndCleanTitle(extractedTitle);
      if (!cleanTitle) {
        new import_obsidian4.Notice("Generated title failed validation");
        return;
      }
      const dir = ((_a3 = file.parent) == null ? void 0 : _a3.path) || "";
      ...
      if (existingFile && existingFile !== file) {
        new import_obsidian4.Notice(`File "${cleanTitle}.${extension}" already exists`);
        return;
      }
      await this.plugin.app.fileManager.renameFile(file, newPath);
      new import_obsidian4.Notice(`File renamed to: ${cleanTitle}.${extension}`);
    } catch (error) {
      console.error("Error generating title:", error);
      new import_obsidian4.Notice(`Failed to generate title using ${this.plugin.settings.provider}. Please check your configuration and try again.`);
    }
  }
};

// src/main.ts
var AutoTitlePlugin3 = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (activeFile) {
          ...
          new import_obsidian5.Notice("No active file to rename");
        }
      }
    });
      ...
      this.app.workspace.on("active-leaf-change", (leaf) => {
        ...
        for (const filePath of this.untitledFiles) {
          if (filePath !== currentActivePath) {
            ...
            if (file instanceof import_obsidian5.TFile && file.basename.includes("Untitled")) {
              setTimeout(() => {
